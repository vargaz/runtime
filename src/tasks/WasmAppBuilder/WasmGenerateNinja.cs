...
public class WasmGenerateNinja : Task
{
    ...
    [NotNull]
    [Required]
    public ITaskItem[]? EmccLDFlags { get; set; }
    [NotNull]
    ...
    public override bool Execute ()
    {
        ...
        var cflags = string.Join (" ", EmccCFlags.Select (s => s.ItemSpec).ToArray ());
        var ldflags = string.Join (" ", EmccLDFlags.Where (s => !s.ItemSpec.EndsWith (".bc\"")).Where (s => !s.ItemSpec.EndsWith (".o\"")).Where (s => !s.ItemSpec.EndsWith (".js\"")).Where (s => !s.ItemSpec.EndsWith (".a\"")).Select (s => s.ItemSpec).ToArray ());

        // Load an extra set of flags from the rsp file
        foreach (var line in File.ReadAllLines (RspFile!)) {
            cflags += " " + line;
            ldflags += " " + line;
        }
        ...
        cflags = cflags.Replace ("-emit-llvm ", "");
        cflags = cflags.Replace ("---profiling-funcs ", "");
        ldflags = ldflags.Replace ("-Oz", "-O0");
        ldflags = ldflags.Replace ("--llvm-opts 2", "-g4");
        ...
        ninja.WriteLine ($"emcc_cflags = {cflags}");
        ninja.WriteLine ($"emcc_ldflags = {ldflags}");
        ...
        foreach (var item in AOTAssemblies) {
            ...
            var bcFile = item.GetMetadata ("LlvmBitcodeFile");
            ninja.WriteLine ($"build {file}.bc: cpifdiff {bcFile}");
